...
{
    int a;
    string filename = "bomb_art.txt";
    printASCII(filename);

    cout << endl
         << "We have 2 mind boggling games for you which will test your intelligence!" << endl
         << "We dare you to challenge yourself and win these games " << endl
         << "Kindly choose any one of the options given below:" << endl
         << "1. Number Tiles: A game where you combine numbered tiles in order to gain a higher numbered tile." << endl
         << "2. Plot 4: A classic strategy game in which player go head-to-head with computer in a battle to own the grid" << endl
         << "3. Exit";
    cout << endl;
}

// ************************************************************Function List for 2048 starts*******************************************************

void upward(int a[4][4])
{
    int i, j, li, ri;
    for (j = 0; j < 4; j++)
    {
...
// *******************************************Fuction List for 2048 ends******************************************************************

// ****************************************Fuction List for plot 4 starts****************************************************************************

void welcome(char &level)
{ // WELCOME MESSAGE TO USER AND SELECT LEVEL............................................

    cout << "\n||Welcome to the Plot 4 Game||\n";
    cout << "\n  Avaliable levels:";
    cout << "\n\t -- E for easy.";
    cout << "\n\t -- H for hard.";
...
void cmpt_choice(vector<vector<char>> &board, int cmpt_input, int &row)
{ // PUTS RANDOM CMPT CHOICE INTO THE BOARD AND CALLS DISPLAY()
...
void cmpt_fun_call(int &cmpt_entry, int &row_1, int &row_2, int &row_3, int &row_4, int &row_5, int &row_6, int &row_7, vector<vector<char>> &board)
{
    ...
    while (test == 0)
    {
        ...
        if (cmpt_entry == 1)
        { // if the coloumn is one from random

            if (row_1 <= 2)
            { // if the row is full we take random again and go to beg. of loop
                srand(time(NULL));
                cmpt_entry = rand() % 7 + 1;
                continue;
            }
            else
            { // if row is not full it calls function which prints onto the board
                cmpt_choice(board, cmpt_entry, row_1);
                test = 1; // set flag as 1 to exit the loop
            }
        }
        else if (cmpt_entry == 2)
        { // if the coloumn is two from random
            if (row_2 <= 2)
            { // if the row is full we take random again and go to beg. of loop
                srand(time(NULL));
                cmpt_entry = rand() % 7 + 1;
        ...
        else if (cmpt_entry == 3)
        { // if the coloumn is three from random
            if (row_3 <= 2)
            { // if the row is full we take random again and go to beg. of loop
                srand(time(NULL));
                cmpt_entry = rand() % 7 + 1;
        ...
        else if (cmpt_entry == 4)
        { // if the coloumn is four from random
            if (row_4 <= 2)
            { // if the row is full we take random again and go to beg. of loop
                srand(time(NULL));
                cmpt_entry = rand() % 7 + 1;
        ...
        else if (cmpt_entry == 5)
        { // if the coloumn is five from random
            if (row_5 <= 2)
            { // if the row is full we take random again and go to beg. of loop
                srand(time(NULL));
                cmpt_entry = rand() % 7 + 1;
        ...
        else if (cmpt_entry == 6)
        { // if the coloumn is six from random
            if (row_6 <= 2)
            { // if the row is full we take random again and go to beg. of loop
                srand(time(NULL));
                cmpt_entry = rand() % 7 + 1;
        ...
        else if (cmpt_entry == 7)
        { // if the coloumn is seven from random
            if (row_7 <= 2)
            { // if the row is full we take random again and go to beg. of loop
                srand(time(NULL));
                cmpt_entry = rand() % 7 + 1;
...
void play_again(int &flag, int &user_entry, int &cmpt_entry, int &row_1, int &row_2, int &row_3, int &row_4, int &row_5, int &row_6, int &row_7, int &counter)
{
    ...
    { // if they choose yes clear out variables
        flag = 1;
        cmpt_entry = 0;
        user_entry = 0;
        row_1 = 8;
...
void userwin_check(const vector<vector<char>> &board, int &win, int &user_check)
...
void cmptwin_check(const vector<vector<char>> &board, int &win, int &user_check)
{ // check if user computer the game
...
int main()
{
    ...
    if (a == 1)
    {
        string filename1 = "Number_Tiles.txt";
        printASCII(filename1);

        // ********************************************* Main Fuction of 2048 starts************************************************************************************
        ...
        // ***********************************************Main Function of 2048 ends***************************************************************
    }

    else if (a == 2)
    {
        string filename2 = "Plot_4.txt";
        printASCII(filename2);

        //  *********************************Main fuction of PLOT 4 Starts*****************************************************************************
        ...
        {                   // while loop flag is one
            ...
            {                   // EASY LEVEL
                ...
                { // when no one has won yet win =0
                    ...
                    if (win != 1)
                    { // only do it when user has not won the game already
                        if (user_entry == 1)
                        {                                // if user chooses coloum 1
                            cmpt_entry = user_entry + 6; // put it into other side of board
                        }
                        else if (user_entry == 7)
                        {                                // if user chooses coloum 7
                        ...
                        if (counter == 42)
                        {                   // if counter is 42 and no one has won then board is full
            ...
            { // HARD LEVEL
                ...
                win_message(win, flag, user_entry, cmpt_entry, row_1, row_2, row_3, row_4, row_5, row_6, row_7, counter);
                // give win message if it is needed ... win lose or draw./.
                main();
            }
            else
            { // if the level entered was not H OR E

                cout << "Inavlid level choice. Please try again."; // give error message
                flag = 1;                                          // flag = 1 so we loop again
            }
        }
        // **************************************Main function of PLOT 4 ends************************************************************
    }
